...
class HeartBeat:
    ...
    def __init__(self, env_path=None, store=True, firedrill_mode=True):
        ...
        )  # pd.Series(index=self.column_names)
    ...
    def make_entry(self, message):
        ...
        if len(detector_df):
            msg["Time After Last"] = (
                msg["Received Times"] - detector_df["Received Times"].max()
            ).total_seconds()
        else:
    ...
    def drop_old_messages(self):
        ...
        locs = np.where(del_t < self.stash_time)[
            0
        ]  # keep only times within stash time e.g. 24 hours
    ...
    def burn_logs(self):
        ...
        if self.store:
            existing_logs = np.array(
                [
                    x
                    for x in existing_logs
                    if (x.endswith("log.json") or x.endswith("log.csv"))
                    and ("complete_heartbeat_log.csv" not in x)
                ]
            )
        ...
        log.debug(
            f"\t> The following logs are older than {self.delete_after} days and will be removed; "
