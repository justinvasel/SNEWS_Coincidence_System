...
detector_file = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "auxiliary/detector_properties.json")
)
...
class FeedBack:
    ...
    def __call__(self):
        ...
        while True:
            ...
            try:
                df = pd.read_csv(
                    mirror_csv,
                    parse_dates=["Received Times"],
                )
            except FileNotFoundError:
                log.error(
                    f"{mirror_csv} does not exist yet! "
                    + "Maybe `snews_cs run-coincidence` is not invoked?"
                )
    ...
    def control(self, df):
        ...
        for detector in data["Detector"].unique():
            ...
            # For a given detector, if already sent an email, ignore the beats before that email.
            # Otherwise, the same cause would ruin the statistics.
    ...
    def check_missed_beats(self, df, detector):
        ...
        if since_lasthb > timedelta(seconds=(mean + 3 * std)):
            ...
            vprint(
                f"[DEBUG] >>>>> Warning for {detector} is created, trying to send.", self.verbose
            )
...
def plot_beats(df, detector, figname):
    ...
    unique_days = list(
        set(
            [
                datetime.strptime(date, "%Y-%m-%d %H:%M:%S.%f").strftime("%Y-%m-%d")
