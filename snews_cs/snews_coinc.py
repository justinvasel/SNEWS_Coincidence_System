...
class CacheManager:
    ...
    def _check_coinc_in_subgroups(self, message):
        ...
        if not is_coinc:
            ...
            temp_cache["neutrino_time_delta"] = (
                pd.to_datetime(temp_cache["neutrino_time_as_datetime"]) - new_ini_t
            ).dt.total_seconds()
    ...
    def _fix_deltas(self, sub_df):
        ...
        sub_df["neutrino_time_delta"] = (
            pd.to_datetime(sub_df["neutrino_time_as_datetime"]) - initial_time
        ).dt.total_seconds()
        ...
        for ind in detector_ind:
            ...
            # update the message if it is coincident with the current sub group
                ...
                self.cache.at[ind, "neutrino_time_delta"] = (
                    message["neutrino_time_as_datetime"] - initial_time
                ).total_seconds()
    ...
    def cache_retraction(self, retraction_message):
        """
        This method handdles message retraction by parsing the cache and dumping any instance of
        the target detector
        ...
        for sub_tag in self.cache["sub_group"].unique():
            ...
            if other_sub["neutrino_time_delta"].min() != 0.0:
                if len(other_sub) == 1:
                    ...
                    other_sub = other_sub.drop(columns=["neutrino_time_delta"])
                    #  make new delta
                    other_sub["neutrino_time_delta"] = (
                        pd.to_datetime(other_sub["neutrino_time_as_datetime"]) - new_initial_time
                    ).dt.total_seconds()
...
class CoincidenceDistributor:
    def __init__(
        ...
        self.alert = AlertPublisher(
            env_path=env_path, use_local=use_local_db, firedrill_mode=firedrill_mode
        )
        if firedrill_mode:
    ...
    def display_table(self):
        ...
        for sub_list in self.coinc_data.cache["sub_group"].unique():
            ...
            sub_df = sub_df.drop(
                columns=["meta", "machine_time", "schema_version", "neutrino_time_as_datetime"]
            )
    ...
    def send_alert(self, sub_group_tag, alert_type):
        ...
        with self.alert as pub:
            ...
            if self.send_email:
                send_email(alert)
            if self.send_slack:
                snews_bot.send_table(alert_data, alert, is_test=True, topic=self.observation_topic)
    ...
    def alert_decider(self):
        ...
        for sub_group_tag, state in self.coinc_data.sub_group_state.items():
            ...
            # publish a retraction alert for the sub group is its state is RETRACTION
                ...
                click.secho(
                    f'SUB GROUP {sub_group_tag}:{"RETRACTION HAS BEEN MADE".upper():^100}',
                    bg="bright_green",
                    fg="red",
                )
                click.secho(
                    f'{"Publishing an updated  alert..".upper():^100}', bg="bright_green", fg="red"
                )
                click.secho(f'{"=" * 100}', fg="bright_red")
            ...
            # Don't publish alert for the sub group is its state is INITIAL
                ...
                click.secho(
                    f'SUB GROUP {sub_group_tag}:{"Initial message recieved".upper():^100}',
                    bg="bright_green",
                    fg="red",
                )
                click.secho(f'{"=" * 100}', fg="bright_red")
                ...
                #  yet another pretty terminal output
                click.secho(
                    f'SUB GROUP {sub_group_tag}:{"A MESSGAE HAS BEEN UPDATED".upper():^100}',
                    bg="bright_green",
                    fg="red",
                )
                click.secho(
                ...
                #  yet another pretty terminal output
                click.secho(
                    f'SUB GROUP {sub_group_tag}:{"NEW COINCIDENT DETECTOR.. ".upper():^100}',
                    bg="bright_green",
                    fg="red",
                )
